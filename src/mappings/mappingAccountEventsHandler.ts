...
export async function newAccountHandler(event: SubstrateEvent): Promise<void> {
    logger.debug('newAccountHandler added'  + JSON.stringify(event?.toHuman()))
...
export async function killedAccountHandler(event: SubstrateEvent): Promise<void> {
    logger.debug('killedAccountHandler added'  + JSON.stringify(event?.toHuman()))
    /* const from = event.event.data[0];
    const to = event.event.data[1];
    const amount = event.event.data[2];
    const txHash = event.extrinsic.extrinsic.hash.toString();
